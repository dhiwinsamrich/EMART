...
const getStudentIndexURL = `${process.env.REACT_APP_API_KEY}/getitemsData?code='Mobiles'`;
...
const[addcart,setAddcart]=useState(false)
  ...
  const handleOnRowClick =(e,item)=>{
    ...
    // <Link to={`MobilePageDetailPage/${item._id}`}/>
...
<Grid item xs={12} sm={6} md={4} key={records.indexOf(item)}>
          <Card sx={{width: '100%', height: '170%', backgroundColor: '#dddddd'}}>
            ...
            <Button sx={{color: '#DDD0C8',backgroundColor: '#323232'}} onClick={() => handleClick(item)}>
            Buy
        </Button>
